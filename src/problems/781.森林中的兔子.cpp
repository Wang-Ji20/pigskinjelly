 ...
 * "还有多少只兔子与你（指被提问的兔子）颜色相同?" ，将答案收集到一个整数数组
...
public:
  int
  numRabbits(vector<int> &answers) {
    unordered_map<int, int> answer_counts;
    for (int answer : answers) {
      answer_counts[answer]++;
    }

    int rabbits = 0;
    for (auto [group, count] : answer_counts) {
      rabbits += ((count + group) / (group + 1)) * (group + 1);
    }
