 ...
 *
 * 比方说，如果 nums = [10, 8, 10, 8] ，那么 h = 9 是一个
 * 合法 整数，因为所有满足 nums[i] > 9 的数都等于 10 ，但是 5 不是 合法 整数。
 *
 * 你可以对 nums 执行以下操作：
 *
 *
 ...
 *
 *
 * 你的目标是将 nums 中的所有元素都变为 k ，请你返回
 * 最少 操作次数。如果无法将所有元素都变 k ，那么返回 -1 。
 *
 *
 *
 * 示例 1：
...
public:
  ...
  minOperations(vector<int> &nums, int k) {
    set<int> nums_dedup;
    for (int num : nums) {
      nums_dedup.insert(num);
    }
    if (*nums_dedup.begin() < k) {
      return -1;
    }
    if (*nums_dedup.begin() == k) {
      return nums_dedup.size() - 1;
    }
    return nums_dedup.size();
  }
};
// @lc code=end
